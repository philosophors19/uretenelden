...
export default function AdminPage() {
  ...
  const handleAddProduct = () => {
    ...
    setFormData({ id: Date.now(), name: "", price: "", stock: 0, category: "", subcategory: "", image: "", info: "" });
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {products.map((p) => (
          <div key={p.id} className="bg-black/30 backdrop-blur-lg rounded-2xl p-4 relative shadow-lg">
            {p.image && (
              <img src={p.image} alt={p.name} className="w-full h-40 object-cover rounded-xl mb-3" />
            )}
            <h3 className="text-xl font-bold">{p.name}</h3>
            <p className="text-gray-300">{p.category} / {p.subcategory}</p>
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black/60 flex justify-center items-center z-50">
          <div className="bg-black/30 backdrop-blur-lg p-6 rounded-xl w-96">
            ...
            <div className="grid grid-cols-1 gap-3">
              <input
                ...
                className="p-2 rounded border border-white/20 bg-black/20"
              />
              <input
                type="text"
                placeholder="Fiyat"
              ...
              {formData.image && <img src={formData.image} alt="Preview" className="w-40 h-40 object-cover rounded-lg mt-2" />}
              <textarea
              ...
              <div className="flex justify-end gap-2 mt-3">
                <button onClick={() => setShowAddModal(false)} className="px-4 py-2 rounded bg-gray-500">İptal</button>
                <button onClick={handleAddProduct} className="px-4 py-2 rounded bg-green-500 hover:bg-green-600">Ekle</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/60 flex justify-center items-center z-50">
          <div className="bg-black/30 backdrop-blur-lg p-6 rounded-xl w-96">
            ...
            <div className="grid grid-cols-1 gap-3">
              ...
              <input type="file" accept="image/*" onChange={(e) => handleImageUpload(e, true)} className="text-white" />
              {editProduct.image && <img src={editProduct.image} alt="Preview" className="w-40 h-40 object-cover rounded-lg mt-2" />}
              <textarea
                value={editProduct.info}
                onChange={(e) => setEditProduct({ ...editProduct, info: e.target.value })}
                className="p-2 rounded border border-white/20 bg-black/20"
              />
              <div className="flex justify-end gap-2 mt-3">
                <button onClick={() => setEditProduct(null)} className="px-4 py-2 rounded bg-gray-500">İptal</button>
                <button onClick={handleSaveEdit} className="px-4 py-2 rounded bg-blue-500 hover:bg-blue-600">Kaydet</button>
              </div>
            </div>
          </div>
        </div>
